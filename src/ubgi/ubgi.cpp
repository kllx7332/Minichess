...
static void do_search(
    ...
    if(movetime_ms > 0){
        int64_t safety_ms = movetime_ms > 500 ? 150 : 40;
        int64_t usable_ms = std::max<int64_t>(1, movetime_ms - safety_ms);
        ctx.use_deadline = true;
        ctx.deadline = steady_start + std::chrono::milliseconds(usable_ms);
    }
    ...
    for(int depth = 1; depth <= depth_limit; depth++){
        ...
        if(multi_pv > 1 && alive()){
            ...
            for(int mpv = 2; mpv <= multi_pv; mpv++){
                ...
                sub_ctx.use_deadline = ctx.use_deadline;
        ...
        if(movetime_ms > 0 && total_ms + 120 >= movetime_ms){
            break;
        }
